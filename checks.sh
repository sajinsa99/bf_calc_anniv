#!/usr/bin/env bash
# Code quality and security checks for bf_calc_anniv.
...
write_report() {
  ...
  ts=$(date '+%Y-%m-%d %H:%M:%S')
  {
    printf '# Checks Report — bf_calc_anniv — %s\n\n' "$ts"
    printf '## Summary\n\n'
    printf '| Check | Status |\n'
    printf '|---|---|\n'
...
section "JSON"
step "jsonlint  calc_anniv_web/package.json" \
  "${DOCKER_RUN[@]}" jsonlint calc_anniv_web/package.json
...
section "JavaScript"
if [[ -f calc_anniv_web/eslint.config.js || -f .eslintrc.json ]]; then
  step "eslint  calc_anniv_web/server.js  calc_anniv_web/public/app.js" \
    "${DOCKER_RUN[@]}" bash -c 'cd calc_anniv_web && npm install --silent && eslint --fix server.js public/app.js'
else
  skip "eslint  (no eslint.config.js found — create one to enable)"
fi

section "YAML"
mapfile -t yaml_files < <(find . -path './.git' -prune -o -path './calc_anniv_web/node_modules' -prune -o \( -name '*.yaml' -o -name '*.yml' \) -print)
...
step "semgrep  calc-anniv.py + JS sources" \
  "${DOCKER_RUN[@]}" semgrep scan --config auto --error \
    calc-anniv.py calc_anniv_web/server.js calc_anniv_web/public/app.js
